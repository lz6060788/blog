...
- [ ] 4.1 创建封面预览组件（components/admin/editor/cover-preview.tsx）
- [ ] 4.2 实现封面图片展示功能，支持来源标签（AI 生成/手动上传）
- [ ] 4.3 实现"生成 AI 封面"按钮，绑定到封面生成 API
- [ ] 4.4 实现"上传封面"按钮，支持文件选择和拖拽上传
- [ ] 4.5 实现"删除封面"功能
- [ ] 4.6 实现"重新生成"按钮
- [ ] 4.7 扩展状态轮询 Hook（hooks/use-ai-generation-status.ts），支持封面状态轮询
- [ ] 4.8 添加封面相关 TypeScript 类型定义
- [ ] 4.9 集成封面预览组件到文章编辑页面
- [ ] 4.10 实现封面生成期间的加载状态展示

## 5. 文章列表和详情页集成

- [ ] 5.1 更新文章列表组件，展示封面图片
- [ ] 5.2 实现封面图片懒加载优化
- [ ] 5.3 更新文章详情页，顶部展示封面图片
- [ ] 5.4 添加封面图片的响应式尺寸支持
- [ ] 5.5 更新页面元数据，使用封面图片作为 Open Graph Image
