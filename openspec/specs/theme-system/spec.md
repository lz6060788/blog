...
#### 场景：使用背景色变量
- **当** 组件需要设置背景色
- **则** 应使用 `bg-theme-bg-canvas`、`bg-theme-bg-surface` 或 `bg-theme-bg-muted`
- **并且** 不应使用硬编码的 Tailwind 颜色类（如 `bg-white`、`bg-zinc-50`）

#### 场景：使用文本色变量
- **当** 组件需要设置文本色
- **则** 应使用 `text-theme-text-canvas`、`text-theme-text-secondary` 或 `text-theme-text-tertiary`
- **并且** 不应使用硬编码的 Tailwind 颜色类（如 `text-black`、`text-white`）

#### 场景：使用强调色变量
- **当** 组件需要设置强调色
- **则** 应使用 `text-theme-accent-primary`、`bg-theme-accent-bg` 等
...
- **当** 深色主题激活
- **并且** 用户查看默认变体按钮
- **则** 按钮背景色应使用深色主题的 `--accent-primary` 变量
